<?php 
	...
	$con = mysqli_connect('localhost', 'root', '', 'vuploads');

	if (isset($_POST['liked'])) {
		$postid = $_GET['video'];
		$result = mysqli_query($con, "SELECT * FROM persons WHERE id=$postid");
		$row = mysqli_fetch_array($result);
		$n = $row['likes'];

		mysqli_query($con, "INSERT INTO likes (userid, postid) VALUES (1, $postid)");
		mysqli_query($con, "UPDATE persons SET likes=$n+1 WHERE id=$postid");
	...
	$posts = mysqli_query($con, "SELECT * FROM persons");
?>
<!DOCTYPE html>
<html lang="en">
<head>
		...
		<?php while ($row = mysqli_fetch_array($posts)) { ?>
			...
			<div class="post">
				...
				<?php 
					...
					$results = mysqli_query($con, "SELECT * FROM likes WHERE userid=2 AND postid=".$row['id']."");
